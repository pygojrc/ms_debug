...
# from the downloaded SDK while ensuring the selected GLib patches are
# compiled into GLib.  The bisect patch is opt-in through FRIDA_DEBUG_BISECT.
...
GLIB_PATCHES=("${ROOT_DIR}/patchs/009-glib-descriptive-thread-name.patch")
if [[ "${FRIDA_DEBUG_BISECT:-0}" == "1" ]]; then
  GLIB_PATCHES=("${ALL_GLIB_PATCHES[@]}")
fi
ARCHIVE="${BUILD_DEPS_DIR}/sdk-${HOST}.tar.xz"

[[ -d "${SDK_DIR}" ]] || { echo "完整 SDK 不存在：${SDK_DIR}" >&2; exit 1; }
for patch in "${ALL_GLIB_PATCHES[@]}"; do
...
# A previous build may have left either selected patch in the cache.  Remove
# only the two patches owned by this script, then apply the current selection.
# Unexpected source edits remain a hard error.
for patch in "${ALL_GLIB_PATCHES[@]}"; do
  if git -C "${GLIB_SRC}" apply --reverse --check "${patch}" 2>/dev/null; then
    git -C "${GLIB_SRC}" apply --reverse "${patch}"
  fi
done
git -C "${GLIB_SRC}" diff --quiet || {
  echo "GLib 源码存在未预期修改，无法应用当前 patch 选择：${GLIB_SRC}" >&2
  exit 1
}
